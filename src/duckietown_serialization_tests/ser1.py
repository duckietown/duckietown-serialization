...
from duckietown_serialization_ds1 import Serializable, Context, GenericData
...
def ser1():
    ...
    context = Context()
    d = ob.as_json_dict(context)
    # from duckietown_serialization_tests import test_dump_and_load
    # test_dump_and_load(d)
    context = Context()
    ...
    ob1 = Serializable.from_json_dict(d2_json, context)

    assert isinstance(ob1.y, MyClass), ob1
    ob2 = Serializable.from_json_dict(d2_yaml, context)
    assert isinstance(ob2.y, MyClass), ob2

    print(ob2)
    ob2_json = json.dumps(ob2.as_json_dict(context), indent=4)
    print(ob2_json)
...
def ser2():
    ...
    ob2.y.x
    w = ob2.as_json_dict(context)
    ob2_json = json.dumps(w, indent=4)
    print(ob2_json)
